...
local function resolve_source(job)
  if not job or not job.file then
    return nil
  end

  if job.file.path then
    return job.file.path
  end

  if job.file.cache then
    return job.file.cache
  end

  if job.file.url and job.file.url.path then
    return job.file.url.path
  end

  if not job.file.url then
    return nil
  end

  local source = tostring(job.file.url)
  if source:match("^search://") then
    source = source:gsub("^search://[^/]+//", "/")
    return Url(source)
  end
...
function M:peek(job)
  local source = resolve_source(job)
  if not source then
    ya.preview_widget(job, { ui.Text({ " [ERROR] cannot resolve archive path" }):area(job.area) })
    return
  end
